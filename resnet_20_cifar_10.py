...
def cal(x, beta = 5, alpha = 0.04, offset = 0.03):
...
def main(args, ITE=0):
    ...
    for run in range(0,10):
        ...
        for _ite in range(args.start_iter, ITERATION):
            ...
            record.to_csv('resnet 20 cifar 10 pro '+str(run)+' '+str(_ite)+'.csv',index=0, header=0)
...
def prune_by_percentile(percent, cycle, model_prev, resample=False, reinit=False, **kwargs):
        ...
        percentile_value = np.percentile(abs(all_weights_curr.flatten() - all_weights_prev.flatten()),percent)
        ...
        for name, param in model.named_parameters():
            ...
            if 'weight' in name:
                ...
                prev_alive_tensor = model_prev[name].cpu().numpy()
                prev_alive = prev_alive_tensor[np.nonzero(prev_alive_tensor)]

                weight_dev = param.device
                new_mask = np.where(abs(tensor-prev_alive_tensor) <= percentile_value, 0, mask[step])
                ...
                #weights_pruned_temp = tensor * np.where(abs(tensor-prev_alive_tensor) > percentile_value, 0, mask[step])
                #if step == 0:
                #    weights_pruned = weights_pruned_temp.reshape(1,-1)
                #else:
                #    weights_pruned = np.hstack((weights_pruned,weights_pruned_temp.reshape(1,-1)))
